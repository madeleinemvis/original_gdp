...
model= gensim.models.KeyedVectors.load_word2vec_format('StanceDetection\google_news\GoogleNews-vectors-negative300.bin.gz',limit=500000, binary=True)
...
def save_predictions(pred, scraped_data_keys):


    with open('./stance_predictions.csv.csv', 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['Body ID', 'Link', 'Stance']
        writer = DictWriter(csvfile, fieldnames=fieldnames, lineterminator='\n')
        writer.writeheader()
        for index, instance in enumerate(scraped_data_keys):
            writer.writerow({'Body ID': index, 'Link': instance, 'Stance': label_ref_rev[pred[index]]})
